    ...
    return (
        <section className="max-w-3xl mx-auto p-6 space-y-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <textarea className="input" placeholder="Short description" required value={description} onChange={(e) => setDescription(e.target.value)} />
                <textarea className="input" placeholder="Full overview" required value={overview} onChange={(e) => setOverview(e.target.value)} />
                <input className="input" placeholder="Venue name" required value={venue} onChange={(e) => setVenue(e.target.value)} />
                <input className="input" placeholder="City, Country" required value={location} onChange={(e) => setLocation(e.target.value)} />
                ...
                <input className="input" placeholder="Target audience" required value={audience} onChange={(e) => setAudience(e.target.value)} />
                <textarea className="input" placeholder="Organizer details" required value={organizer} onChange={(e) => setOrganizer(e.target.value)} />
                ...
                <div>
                    ...
                    {tags.map((t, i) => (
                        <div key={i} className="flex gap-2 mb-2">
                            <input className="input flex-1" placeholder="Tag" value={t} onChange={(e) => updateTag(i, e.target.value)} />
                        </div>
                    ))}
                ...
                {/* ----- dynamic agenda ----- */}
                <div>
                    <label className="block mb-2 font-semibold">Agenda</label>
                    {agenda.map((a, i) => (
                        <div key={i} className="flex gap-2 mb-2">
                            <input className="input flex-1" placeholder="Agenda item" value={a} onChange={(e) => updateAgenda(i, e.target.value)} />
                        </div>
                    ))}
                    <button type="button" className="btn-secondary" onClick={addAgenda}>+ Add Item</button>
                </div>
